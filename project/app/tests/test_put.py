...
def put_request(payload = create_payload()):
...
def wait_for_api(timeout=15):
    for _ in range(timeout):
        try:
            response = requests.get(f"{BASE_URL}/users")  # or your health endpoint
...
def test_no_data():
    ...
    response = put_request(payload)
    assert response.status_code == 400

def test_similiar_email():
    payload = create_payload()
    payload.update({"email": "test01@example.com"})
...
def test_update_user():
    payload = create_payload()
    payload.update({"email": "test02@example.com"})
